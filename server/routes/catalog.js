...
const COLLECTION_COLS = ['user_id', 'owned', 'card_number', 'set_name', 'description',
  'team_city', 'team_name', 'rookie', 'auto', 'mem', 'serial', 'serial_of',
  'thickness', 'year', 'product', 'grade', 'duplicates'];
const COLLECTION_CHUNK = Math.floor(65534 / COLLECTION_COLS.length); // ~3855

// POST /api/catalog/add-to-collection — user: copy a catalog set into their cards (streams NDJSON progress)
router.post('/add-to-collection', async (req, res) => {
  ...
  const toInsert = catalogCards.map(c => ({
    user_id: uid, owned: 0, card_number: c.card_number, set_name: c.set_name,
    description: c.description, team_city: c.team_city, team_name: c.team_name,
    rookie: c.rookie, auto: c.auto, mem: c.mem, serial: c.serial, serial_of: c.serial_of,
    thickness: c.thickness, year: c.year, product: c.product, grade: c.grade, duplicates: 0,
  }));
  ...
  try {
    await db.begin(async sql => {
      ...
      let done = 0;
      for (let i = 0; i < total; i += COLLECTION_CHUNK) {
        await sql`INSERT INTO cards ${sql(toInsert.slice(i, i + COLLECTION_CHUNK), ...COLLECTION_COLS)}`;
        done = Math.min(i + COLLECTION_CHUNK, total);
        send({ progress: Math.round(done / total * 100), done, total });
      }
    });
    send({ added: total });
  } catch (err) {
    console.error('Add-to-collection error:', err);
    send({ error: 'Failed to add set to collection.' });
  }
  res.end();
});
